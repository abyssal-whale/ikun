...
def Bvlst2Avlst(bvlst):
    # 将Bv号列表转化为的Av号
    avlst=[]
    for i in range(0,len(bvlst)):
        avlst.append(Bv2Av(bvlst[i]))
    return avlst
...
if __name__ == '__main__':
    """
    Bv = input("请输入Bv号:")
    print(Bv + "的Av号为:av" + str(Bv2Av(Bv)))
    print("链接地址为：http://www.bilibili.com/video/av" + str(Bv2Av(Bv)))
    """
    bvlist=getBvByKeyword.getBvByKey(KEY_WORD)
    print(Bvlst2Avlst(bvlist))
